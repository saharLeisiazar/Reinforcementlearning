...
class replay_buffer:
    ...
    def add_sample(self, s, angle, vel, new_s, r, terminal = 0):
    ...
    def sample_recent_data(self, Batch_size = 1):
        if len(self.rollout_list) > 3* Batch_size :
            self.rollout_list = self.rollout_list[-3*Batch_size:]
    ...
    def convert_rollouts_to_list(self, rollouts):
        ...
        angle_b = np.array([rollout["angle"] for rollout in rollouts]) 
        vel_b = np.array([rollout["velocity"] for rollout in rollouts])
        ...
        return ob_b, angle_b, vel_b, next_ob_b, r_b, t_b    
