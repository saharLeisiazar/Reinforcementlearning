...
class RL_Trainer(object):
    ...
    def perform_dqn_logging(self, itr, all_logs):
        ...
        if (itr% 1000 == 0 or itr == self.n_iter-1):
            self.itr_list.append(itr+1)
            self.best_mean_reward_list.append(np.mean(self.best_mean_episode_reward))
            self.mean_reward_list.append(np.mean(self.mean_episode_reward))
