...
class FFModel(nn.Module, BaseModel):
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        target = next_observations - observations# TODO(Q1) compute the normalized target for the model.
        target = normalize(target, data_statistics['delta_mean'], data_statistics['delta_std'])
        
        next_obs_pred, delta_pred_normalized = self.forward(observations, actions, **data_statistics)
        # Hint: you should use `data_statistics['delta_mean']` and
        # `data_statistics['delta_std']`, which keep track of the mean
        # and standard deviation of the model.

        loss = self.loss(delta_pred_normalized, ptu.from_numpy(target)) # TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
